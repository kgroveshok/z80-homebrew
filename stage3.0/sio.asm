...
;notsame$:	db 10,13,"Data NOT same as previous read",10,13,
...
; memory is now all RAM
	ld sp,stackp 	;set up stack below monitor
	ld hl,signon
	call strout
...
cp 'D'
        ...
        CP 'E'
        JP Z,EDMEM
        cp 'I'         ; read data from specified I/O port in page 0
	jp z,INPORT
	cp 'O'		; write data to specified I/O port in page 0
	jp z,OUTPORT
	cp 'L'		; list memory as Intel Hex format
	jp z,LISTHEX
        	CP 'G'
        	JP Z,go
...
what:
        	...
        	jr CMD
abort:
	ld hl,abortstr	; print command not executed
	call strout
	jr CMD
...
; 
	ld hl,inportstr	; print command 'I' prompt
	call strout
...
; command format is "O value port#"
	ld hl,outportstr	; print command 'O' prompt
	call strout
	...
	ld hl,outport2str	; print additional prompt for command 'O'
	call strout
...
; print help message
HELP:
	ld hl,HELPstr	; print help message
	call strout
...
go:
	ld hl,gostr		; print go command message
	call strout
        	CALL ADRIN
	jp z,CMD		;return to command prompt if illegal input
        	LD H,D
...
#done$:	db " done",0
abortstr:	db 13,10,"command aborted",0
gostr:	db "o to address: 0x",0
inportstr:	db "nput from port ",0
invaluestr:	db 10,13,"Value=",0
outportstr:	db "utput ",0
outport2str:	db " to port ",0
listhexstr:	db "ist memory as Intel Hex, start address=",0
listhex1str:	db " end address=",0
