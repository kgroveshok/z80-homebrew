...
exec1:
	ld (os_tok_ptr), hl                 ; here move out of repeated saving in this tight loop

	; copy our PC to working vars 
	ld (cli_ptr), hl                    ; here
	ld (cli_origptr), hl                ; here

	ld a,(hl)                           ; here make hl be the ram ptr
	cp FORTH_END_BUFFER
	ret z
...
ld hl, baseram
;ld hl, sysdict
ld (cli_nextword),hl                            ; here do we need to do this?
;           match word at pc
;           exec word
;           or push to dsp
...
.execpnword:	; HL at start of a word in the dictionary to check
	...
	ld hl,(cli_nextword)                  ; here remove the need to store????
...
.execpnchar:    ; compare char between token and string to parse
	...
	;ld hl, (cli_token)     ; the dict word 
	ld de, (cli_ptr)     ; cli to parse


.execpncharl:    ; compare char between token and string to parse (loop)
...
;	cp 0			 ; add both chars together, if 32 then other must be 0 so at end of string we are parsing?
	...
	jr nz, .execpncharl 		 ; not at end of strings yet
